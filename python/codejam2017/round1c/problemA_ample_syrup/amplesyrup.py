...
class AmpleSyrup:
    ...
    def solve(n, k, pancakes):
        pancakes = sorted(pancakes, key=lambda x: -AmpleSyrup.get_first_pancake_surface(x))
        surface = AmpleSyrup.get_first_pancake_surface(pancakes[0])

        pancakes = sorted(pancakes[1:], key=lambda x: -AmpleSyrup.get_second_pancake_surface(x))
        for i in range(0, k-1):
            surface += AmpleSyrup.get_second_pancake_surface(pancakes[0])
